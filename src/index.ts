...
const server = new McpServer(
  ...
  {
    instructions: `Bridge to the Fatture in Cloud API v2 (Italian invoicing platform). Read and write (CRUD) on issued documents (invoices, credit notes...) and received documents (expenses...).

If FIC_COMPANY_ID is set, tools use it automatically; otherwise call list_companies first to discover the company_id.
...
const companyIdSchema = z
  ...
  .optional()
  .describe(
    "Fatture in Cloud company ID. Omit to use the FIC_COMPANY_ID env var; discover IDs with list_companies."
  );
